...
public class GiantDesertTorchFeature extends AbstractTemplateFeature<NoFeatureConfig> {
    ...
    public boolean generate(ISeedReader world, ChunkGenerator chunkGenerator, Random rand, BlockPos pos, NoFeatureConfig config) {
        ...
        mutable.setPos(cornerPos).move(Direction.SOUTH, 1).move(Direction.EAST, 2);
        if (world.isAirBlock(mutable)) return false;

        mutable.setPos(cornerPos).move(Direction.SOUTH, 2).move(Direction.EAST, 1);
        if (world.isAirBlock(mutable)) return false;
